...
class dnbGame():
    ...
    def __init__(self, boardSize, playerName, difficulty,
                 distanceBetweenDots, markerSizeInCM, frameWidth, 
                 frameHeight, cameraMatrix = cameraMatrix, distCoeffs = distCoeffs, 
                 firstPlayer = 0):
    ...
    def detect_board(self, frame):
        ...
        if ids is not None:
            ...
            if (0 in ids) and (1 in ids) and (2 in ids) and (3 in ids) and len(ids) == 4: # This is done to avoid false detections (for example, [2,1,3,38])
                ...
                tcpProjected0, _ = cv2.projectPoints(t0p, markersPoseSorted[0][0], markersPoseSorted[0][1], cameraMatrix, distCoeffs)
                ...
                # of the 3D translation vector (from the camera to the circle) to the image plane (2D projection)
                tcpProjected1, _ = cv2.projectPoints(t1p, markersPoseSorted[1][0], markersPoseSorted[1][1], cameraMatrix, distCoeffs)
                ...
                # of the 3D translation vector (from the camera to the circle) to the image plane (2D projection)
                tcpProjected2, _ = cv2.projectPoints(t2p, markersPoseSorted[2][0], markersPoseSorted[2][1], cameraMatrix, distCoeffs)
                ...
                # of the 3D translation vector (from the camera to the circle) to the image plane (2D projection)
                tcpProjected3, _ = cv2.projectPoints(t3p, markersPoseSorted[3][0], markersPoseSorted[3][1], cameraMatrix, distCoeffs)
                ...
                frameTestMarkerstoPoints = cv2.warpPerspective(frameCopy,transformationMatrix,(self.frameWidth,self.frameHeight))
        ...
        return frameCopy, framewithOverlay, boardFrame, frameTestMarkerstoPoints, boardWithDotsFrame, boardWithDotsAverageFrame
